...
export async function GET(
  ...
  try {
    ...
    console.error("[api/appointments/[id] GET] error", err);
    return NextResponse.json(
  ...
  try {
    ...
    if (typeof body?.status === "string") {
      if (!VALID_STATUSES.has(body.status)) {
        return NextResponse.json(
          { ok: false, error: "Statut invalide. Valeurs autorisées : scheduled, confirmed, completed, cancelled." },
          { status: 400 }
        );
      }
    ...
    if (data.status && data.status !== currentAppointment.status) {
      ...
      await db.leadActivity.create({
        data: {
          ...
          oldValue: currentAppointment.status,
    ...
    console.error("[api/appointments/[id] PATCH] error", err);
    return NextResponse.json(
      { ok: false, error: "Erreur serveur" },
      { status: 500 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    console.error("[api/appointments/[id] DELETE] error", err);
    return NextResponse.json(
      { ok: false, error: "Erreur serveur" },
      { status: 500 }
    );
